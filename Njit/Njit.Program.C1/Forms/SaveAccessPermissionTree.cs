...
namespace Njit.Program.ComponentOne.Forms
{
    public partial class SaveAccessPermissionTree : Form
    {
        ...
        private void AddFormsThatIsNotExist(Type[] types)
        {
            foreach (Type type in types)
            {
                ...
                if (!FormExistInNodes(treeView.Nodes, form))
                {
                    ...
                    IEnumerable<Njit.Common.IAccessPermission> fields = GetFields(form);
                    foreach (Njit.Common.IAccessPermission control in fields)
                    {
        ...
        private IEnumerable<Njit.Common.IAccessPermission> GetFields(Form control)
        {
            List<Njit.Common.IAccessPermission> list = new List<Njit.Common.IAccessPermission>();
            foreach (Control c in control.Controls)
            {
                if (c.GetType().GetInterface(typeof(Njit.Common.IAccessPermission).FullName) != null)
                    list.Add(c as Njit.Common.IAccessPermission);
                if (c is C1.Win.C1Ribbon.C1Ribbon)
                    GetFields(c as C1.Win.C1Ribbon.C1Ribbon, list);
                else
                    GetFields(c, list);
            }
            return list;
        ...
        private void GetFields(C1.Win.C1Ribbon.C1Ribbon ribbon, List<Njit.Common.IAccessPermission> list)
        {
            foreach (C1.Win.C1Ribbon.RibbonTab tab in ribbon.Tabs)
            {
                if (tab.GetType().GetInterface(typeof(Njit.Common.IAccessPermission).FullName) != null)
                    list.Add(tab as Njit.Common.IAccessPermission);
                foreach (C1.Win.C1Ribbon.RibbonGroup group in tab.Groups)
                {
                    if (group.GetType().GetInterface(typeof(Njit.Common.IAccessPermission).FullName) != null)
                        list.Add(group as Njit.Common.IAccessPermission);
                    foreach (C1.Win.C1Ribbon.RibbonItem item in group.Items)
                    {
                        if (item.GetType().GetInterface(typeof(Njit.Common.IAccessPermission).FullName) != null)
                            list.Add(item as Njit.Common.IAccessPermission);
                    }
                }
            }
        }

        private void GetFields(Control control, List<Njit.Common.IAccessPermission> list)
        {
            foreach (Control c in control.Controls)
            {
                if (c.GetType().GetInterface(typeof(Njit.Common.IAccessPermission).FullName) != null)
        ...
        private object GetControl(Form form, string name)
        {
            ...
            var query = fields.Where(t => t.GetPath() == name);
            if (query.Count() == 1)
                return query.Single();
            return null;
        }
